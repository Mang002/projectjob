...
import { Video } from "expo-av"; // นำเข้า Video จาก expo-av
import { MaterialIcons } from "@expo/vector-icons";

const API_URL = "https://localwisdom.online/app";

const CraftDetails = ({ route }) => {
  const { craftId } = route.params;
  ...
  const fetchFoodData = async () => {
    try {
      const response = await fetch(`${API_URL}/craft.php?id=${craftId}`);
  ...
  const fetchComments = async () => {
    try {
      const response = await fetch(`${API_URL}/comments_craft.php?id=${craftId}`);
      const data = await response.json();
      setComments(data);
  ...
  const fetchLikes = async () => {
    try {
      const response = await fetch(`${API_URL}/like_craft.php?id=${craftId}`);
      const data = await response.json();
      setLikes(data.likes);
    } catch (error) {
  ...
  const handleLike = async () => {
    try {
      ...
      const response = await fetch(
        `${API_URL}/likes_craft.php?id=${craftId}&uid=${userId}`,
        {
          method: "POST",
        }
      );
  ...
  const handleAddComment = async () => {
    if (comment.trim()) {
      try {
        const userId = await AsyncStorage.getItem("id");
        const response = await fetch(
          `${API_URL}/add_comCra.php?id=${craftId}&uid=${userId}`,
          {
            method: "POST",
            headers: {
              "Content-Type": "application/json",
