...
const FoodDetail = ({ route }) => {
  ...
  const fetchFoodData = async () => {
    try {
      const response = await fetch(`${API_URL}/food.php?id=${foodId}`);
  ...
  const fetchComments = async () => {
    try {
      const response = await fetch(`${API_URL}/comments.php?id=${foodId}`);
      const data = await response.json();
      setComments(data);
  ...
  const fetchLikes = async () => {
    try {
      const response = await fetch(`${API_URL}/like_food.php?id=${foodId}`);
      const data = await response.json();
      setLikes(data.likes);
    } catch (error) {
  ...
  const handleLike = async () => {
    try {
      ...
      const response = await fetch(
        `${API_URL}/likes.php?id=${foodId}&uid=${userId}`,
        {
          method: "POST",
        }
      );
  ...
  const handleAddComment = async () => {
    if (comment.trim()) {
      try {
        ...
        console.log(userId);
        const response = await fetch(
          `${API_URL}/add_comment.php?id=${foodId}&uid=${userId}`,
          {
            method: "POST",
            headers: {
              "Content-Type": "application/json",
    ...
    <View key={index} style={styles.commentContainer}>
      ...
      {item.reply_content && (
        <View style={[styles.commentBubble, styles.adminReply]}>
          ...
          <View style={styles.divider} />
          <Text style={styles.replyAuthor}>แอดมิน • {item.reply_created_at}</Text>
        </View>
      )}
    </View>
  );
  ...
  return (
    <ScrollView style={styles.container}>
      <View style={styles.card}>
        <Image source={{ uri: 'https://localwisdom.online/admin/' + foodData?.imageUrl }} style={styles.image} />
